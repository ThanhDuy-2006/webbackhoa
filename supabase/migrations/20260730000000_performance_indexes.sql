-- Migration: Safe & Verified Performance Partial Indexes for Storefront Queries

-- 1. Storefront Products Index (ordered by created_at DESC for newest filtering)
CREATE INDEX IF NOT EXISTS idx_products_storefront_created_at
ON public.products (created_at DESC)
WHERE deleted_at IS NULL AND is_active = true;

-- 2. Storefront Products Category Filtering Index
CREATE INDEX IF NOT EXISTS idx_products_storefront_category_created_at
ON public.products (category_id, created_at DESC)
WHERE deleted_at IS NULL AND is_active = true;

-- 3. Storefront Categories Navigation Index
