...
import { bulkCreateSellerProductsAction } from '@/actions/seller-product.actions'
...
export function SellerProductImportClient({ categories, initialScan = false }: Props) {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (createdCount > 0) {
          msg = `Đã thêm mới ${createdCount} sản phẩm thành công!`
        }
